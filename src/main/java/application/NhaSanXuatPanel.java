...
import businesslayer.MangNhaSanXuat;
...
public class NhaSanXuatPanel extends JPanel  implements ActionListener,MouseListener,WindowListener,KeyListener{
        ...
        private MangNhaSanXuat NSXArr = new MangNhaSanXuat();
        private int soLuongRow=20,isSr;
        private InfoNSX addNSX;
        private String[] title=NSXArr.title();
        private ImageIcon iconEdit,iconDelete,iconSr,iconPlus,iconFile;
        private Color mainColor=new Color(0xF5F5F5);
        private ArrayList<NhaSanXuat> dsNSX=NSXArr.getNsxArr();
        NhaSanXuatPanel() {
            ...
            heading=new JLabel("Danh sách nhà sản xuất");
    ...
    private void getTableData(int soLuongRow, int trang) {
            ...
            DSNSX=NSXArr.get(soLuongRow,trang);
            int length=DSNSX.size();
            for (int i=0;i<length;i++) {
                Vector row=new Vector();
                row.add(soLuongRow*(trang-1)+i+1);
                row.add(DSNSX.get(i).getMaNSX());
                row.add(DSNSX.get(i).getTenNSX());
                row.add(DSNSX.get(i).getDiaChi());
                row.add(DSNSX.get(i).getSoDienThoai());
                model.addRow(row);
            }*/
            int length=dsNSX.size();
    ...
    public void actionPerformed(ActionEvent e) {
            ...
            case "prev":
                ...
                break;
            case "next": {
                Double trangCuoi = NSXArr.getTongSoTrang(soLuongRow);
                Double trangHienTai = Double.parseDouble(number.getText());
                Double length=dsNSX.size()-trangHienTai*soLuongRow;
            ...
            case "excel": {
                CreateExcelFile.fileNSX(NSXArr.getNsxArr());
                break;
            }
         }
    }
    ...
    public void timKiem() {
         ...
         NSXSr=NSXArr.search(text.getText());
         /*NhaSanXuat NCCSR=NCCArr.search(Integer.parseInt(text.getText()));
                ...
                for (NhaSanXuat nsx: NSXSr) {
                    Vector row=new Vector();
                    row.add(1);
                    row.add(nsx.getMaNSX());
                    row.add(nsx.getTenNSX());
                    row.add(nsx.getDiaChi());
                    row.add(nsx.getSoDienThoai());
                    dtmSr.addRow(row);
                }
                jt.setModel(dtmSr);
                retable();
    ...
    public void them() {
        addNSX=new InfoNSX();
        addNSX.add(); 
        addNSX.addWindowListener(this);
        addNSX.setVisible(true);      
    }
    
    public void sua() {
        addNSX=new InfoNSX(); 
        int x=jt.getSelectedRow();
        addNSX.edit(Integer.parseInt(jt.getModel().getValueAt(x,1).toString()),jt.getModel().getValueAt(x,2).toString(),
    ...
    public void xoa() {
        ...
        if (result==0) {
            int x=jt.getSelectedRow();
            NSXArr.remove(Integer.parseInt(jt.getModel().getValueAt(x,1).toString()));
            int trang=Integer.parseInt(number.getText());
            NSXArr=new MangNhaSanXuat();
            dsNSX=NSXArr.getNsxArr();
    ...
    public void windowClosed(WindowEvent e) {
            if (e.getSource()==addNSX && addNSX.getAlter()==true) {
                    if (isSr==0) {
                        int trang=Integer.parseInt(number.getText());
                        NSXArr=new MangNhaSanXuat();
                        dsNSX=NSXArr.getNsxArr();
